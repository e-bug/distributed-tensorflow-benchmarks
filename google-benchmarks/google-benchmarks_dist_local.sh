...
for running_ps in `seq 1 $TF_NUM_PS`; do
  ps_idx=$(($running_ps - 1))
  gnome-terminal -x sh -c "./$RUN_TF_SCRIPT ps $ps_idx $TF_PS_HOSTS $TF_WORKER_HOSTS $3 $4; bash"
done
...
for running_w in `seq 1 $((TF_NUM_WORKER - 1))`; do
  w_idx=$(($running_w - 1))
  gnome-terminal -x sh -c "./$RUN_TF_SCRIPT worker $w_idx $TF_PS_HOSTS $TF_WORKER_HOSTS $3 $4; bash"
done
w_idx=$running_w
./$RUN_TF_SCRIPT worker $w_idx $TF_PS_HOSTS $TF_WORKER_HOSTS $3 $4
